...
class d_controls
{
	...
	var $auto_commit_transaction = true;
        ...
        var $load_by_tx_id=true;
        var $main_table_id = -1;
        var $insert_main_tx = true;
        var $main_table_id_column="id";
	...
	private function LoadEditValues()
	{
		...
		{                    
			$table_name = $this->pages_row[0]['page_type_value'];       
                        if($this->edit_query=="") $this->edit_query = orm::GetSelectQuery($table_name,array(),array($this->GetUpdateColumn()=>$this->edit_id), "");                        
			$this->edit_rows = $this->db->query_as_array($this->edit_query);                       
		}
		else
		{                            
			if($this->load_by_tx_id==true) $this->edit_rows = $this->db->query_as_array(orm::GetSelectQuery("d_values",array(),array("tx_id"=>$this->edit_id), ""));
                        else $this->edit_rows = $this->db->query_as_array(d_queries::get_values($this->main_table_id));                                                
		}                                 
                
                if(count($this->edit_rows)==0 && $this->pages_row[0]['success_url']!="") util::redirect ($this->ReplaceUrl($this->pages_row[0]['success_url'])) ;                                               
	}
	
	public function DrowHtml($drow_buttons=false,$drow_table=true)
	{	
                $html="";
                global $D_CONTROLS;
		...
		if($drow_table==true) $html = "<table class=d_main_table border=0>";
		...
		{
			$html.="<tr><td colspan=2 align=left><br /><input style='display:none' class=btn id=btnWait type=button value='".WAIT."' /><input type=submit class=btn name=btnSave id=btnSave value='".SAVE."' onclick='return checkform();'>&nbsp;<input class=btn onclick='javascript:history.back(1)' type=button value='".CANCEL."' ></td></tr>";
		}
		if($drow_table==true) $html.="</table>";
		$this->chtml = $html;
	...
	private function get_control_html($row)
	{
		...
		{			
                        $id_column = "id";
                        $value_clumn = "value_text";
			$control_id = "drp".$row['id'];
			if($this->edit_mode) $control_value = $this->GetValueFromDB($row); 
                        
                        if($row['c_mode']=="2")
                        {
                            $c_settings = explode(":",$row['c_type_param1']);                            
                            $query = orm::GetSelectQuery($c_settings[0], array(), array(), $c_settings[2]);
                            $results = $this->db->query_as_array($query);                                
                            $value_clumn = "fa_name";
                        }
                        else {
			...
			$html.= webcontrols::GetDropDown2($control_id, $results , $id_column, $value_clumn,$control_value,"",$row['c_style_name'],$add_not_selected,$translate_array);
	...
	public function SaveData($check_btn_click=true, $redirect=true, $main_table_id=-1)
	{				
		if($check_btn_click)
		{
		...
		try
                {                    
                        if(!$this->edit_mode)
                        {                    
                            $sql = orm::GetInsertQuery("d_txs", array("page_id"=>$this->page_id ,"branch_id"=>$this->current_branch_id, "inserted_by"=>$this->current_user_id, "inserted_date"=>util::Now(), "tbl_id"=>$main_table_id));			
                            if($this->insert_main_tx) $last_id = $this->db->insert_query($sql);                            
                        }
                        else
                        {
                            if($this->load_by_tx_id==false) $this->edit_id = $this->db->query_single_value(orm::GetSelectQuery("d_txs",array("id"),array("page_id"=>$this->page_id,"tbl_id"=>$this->main_table_id),""),"id");                            
                            $sql = orm::GetUpdateQuery("d_txs", array("updated_by"=>$this->current_user_id, "updated_date"=>util::Now()), array("id"=>$this->edit_id));
                            if($this->insert_main_tx) $this->db->query($sql);
	...
	private function SaveTableBasedValues()
	{
		...
		$sql = "";		
                ...
                if($this->insert_user_info)
                {
                     ...
                     $columns_arr["branch_id"] = $this->current_branch_id;
                }
		...
		{                    
                    ...
                    if($this->insert_main_tx) $columns_arr['tx_id'] = $this->tx_id;                    
                    $sql = orm::GetInsertQuery($table_name,$columns_arr);
		}
		else
		{       
                    $sql = orm::GetUpdateQuery($table_name,$columns_arr, array($this->GetUpdateColumn()=>$this->tx_id));
		}                
        ...
        private function GetUpdateColumn()
        {
            return $this->insert_main_tx==true ? "tx_id" : $this->main_table_id_column;
        }  
...
class d_queries
{
	...
	public static function get_page($page_id)
	{
        ...
        public static function get_values($main_table_id)
        {
            $sql = "select * from d_values v 
                    inner join d_txs tx on tx.id=v.tx_id
                    where tx.tbl_id=$main_table_id
                    " ;
            return $sql;        
        }
