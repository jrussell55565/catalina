...
{
    if(rtrim(ltrim(sText))=="") return true;
    return false;
}
...
{        
	...
	val = trim(val);          
        ...
        var check_empty = settings[i]=="0" && trim(val)=="" ? false : true;          
    //    if(settings[i]=="1" && val == "")
    //    {
     //       error_msg+="Cannot be empty \n"; 
     //   }
     
        if(settings[i]=="1" && trim(val)=="")
        {            
            error_msg+=errmsgs[i]+"\n";
        }
        else {
	...
	else if(modes[i]=="an")
	{
        ...
        else if(modes[i]=="regexp")
	{                 
           // alert(check_empty);
           // alert(val.match(regexps[i]));
            if(!val.match(regexps[i]) && check_empty)
            {                                
                error_msg+=errmsgs[i]+"\n";                
            }    
	}
