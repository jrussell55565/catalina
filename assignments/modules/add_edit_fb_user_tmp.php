...
<table >
    <tr>
        ...
        <td><input class='form-control' type="text" id="txtName" disabled="disabled" name="txtName"  value="<?php echo util::GetData("txtName") ?>" /></td>
    </tr>
    <tr>
        <td><?php echo USER_SURNAME ?> : </td>
        <td><input class='form-control' type="text" id="txtSurname"  disabled="disabled" name="txtSurname" value="<?php echo util::GetData("txtSurname") ?>" /></td>
    </tr>
    <tr>
...
<tr>
    ...
    <tr>
        <td colspan="2" align="center">
            ...
            <input class="btn green" type="button" name="btnCancel" value="<?php echo CANCEL ?>" id="btnCancel" onclick="javascript:window.location.href='?module=fb_users'" />
        </td>
    </tr>
</table>
...
function checkform()
{
    ...
    else
    {
        ...
        if(status)
        {
             $.post("?module=add_edit_fb_user", { email_to_check: email, ajax: "yes" },
             function(data){
                 if(data=="0")
                 {
                     return true;
