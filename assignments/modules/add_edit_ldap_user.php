...
<?php
   ...
   // $val->AddValidator("txtEmail", "email", R_EMAIL_VAL , "-1");
    ...
    if(isset($_GET["id"]))
    {
        access::has("edit_ldap_user",2);        
        $login_disabled="disabled='disabled' ";
        $mode="edit";
        $id = util::GetID("?module=ldap_users");
        $rs_users=orm::Select("app_users", array(), au::arr_where(array("UserID"=>$id)), "");

        if(db::num_rows($rs_users) == 0 ) util::redirect("?module=ldap_users");
    ...
    else access::has("add_ldap_user",2);        
    ...
    if(isset($_POST["btnSave"]) && $val->IsValid())
    {
        ...
        util::redirect("?module=ldap_users");
    }
