...
<?php
    ...
    $grd = new extgrid($hedaers,$columns, "index.php?module=fb_users");
    $grd->exp_headers = $exp_hedaers;
    $grd->exp_columns = $columns;
    $grd->edit_link="index.php?module=add_edit_fb_user";
    $grd->id_column="UserID";
    $grd->column_override=array("type_name"=>"user_type_override");
    $grd->auto_id=true;
    //$grd->modify_system_rows=false;
    $user_quizzes = access::has("fb_usr_dtls") ? DETAILS : "";
    $ip_rests  = access::has("fb_ip_res") ? IP_RESTRCITIONS : "";    
    $grd->id_links=array($ip_rests=>"?module=ip_res&type=3",$user_quizzes=>"?module=user_details");
    
    if(!access::has("delete_fb_user")) $grd->delete_text = "";
    if(!access::has("edit_fb_user")) $grd->edit_text = "";
    ...
    if($grd->IsClickedBtnDelete() && access::has("delete_fb_user"))
    {            
    ...
    $query = users_db::GetAppUsersQuery("",au::get_where(true), $grd->GetSortQuery());
