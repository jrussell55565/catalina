...
if ($_POST['btn_csa'])
{
  try
  {
    ...
    $statement = "load data local infile \"".$_FILES['file_csa']["tmp_name"]."\"
               ...
               (employee_id,@v_date,state,number,level,placard_inspection,hm_inspection,basic,violation_group,code,description,out_of_service,convicted_diff_charge,violation_weight,time_weight,total_points,points_cash_value,basic_violation_inspection,last_name,first_name,co_driver_last_name,co_driver_first_name,@v_excel_date)
               SET date = str_to_date(@v_date, '%m/%d/%Y'), import_date = str_to_date(@v_excel_date, '%m/%d/%Y')";
