...
from (
select past_and_current_violations.*, users.status, concat_ws(' ', users.fname, users.lname) AS real_name from (
select
...
SELECT
 ...
 LEFT JOIN (SELECT
  ...
  ON past_violations.employee_id = current_violations.employee_id ) past_and_current_violations
RIGHT OUTER JOIN users ON users.employee_id = past_and_current_violations.employee_id) past_and_current_plus_users,
(SELECT *
         FROM cp_csa) cp_csa;
