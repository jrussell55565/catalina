      ...
      (
        ...
        FROM (
               SELECT
                 ...
                 round(TIMESTAMPDIFF(DAY, str_to_date(v_date_start, '%Y-%m-%d'), str_to_date(v_date_end, '%Y-%m-%d')) * .675,
               ...
               FROM (
                      ...
                      FROM ((
                              ...
                              FROM tasks
                              WHERE
                                submit_date BETWEEN STR_TO_DATE(v_date_start, '%Y-%m-%d') AND STR_TO_DATE(v_date_end, '%Y-%m-%d')
                              GROUP BY assign_to) tasks LEFT OUTER JOIN (
                                                                          ...
                                                                          WHERE uq.added_date BETWEEN STR_TO_DATE(v_date_start,
                        ...
                        LEFT OUTER JOIN (
                                          ...
                                          FROM days_worked
                                          WHERE worked = 1 AND
                                                `date worked` BETWEEN STR_TO_DATE(v_date_start, '%Y-%m-%d') AND STR_TO_DATE(
                                                    v_date_end, '%Y-%m-%d')
                        ...
                        LEFT OUTER JOIN (
                                          ...
                                          WHERE (
                                            began BETWEEN STR_TO_DATE(v_date_start, '%Y-%m-%d') AND STR_TO_DATE(v_date_end,
                                                                                                         '%Y-%m-%d')
                                            AND Ended BETWEEN STR_TO_DATE(v_date_start, '%Y-%m-%d') AND STR_TO_DATE(v_date_end,
                                                                                                             '%Y-%m-%d'))
